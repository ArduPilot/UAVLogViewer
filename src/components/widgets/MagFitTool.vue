...
function simulatedAnnealing (f, xStart, bounds, { t = 100, tMin = 1e-6, alpha = 0.9, maxIter = 1000 } = {}) {
    const n = xStart.length
    let x = [...xStart]
    let fBest = Infinity
    let xBest
    ...
    for (let iter = 0; iter < maxIter && t > tMin; iter++) {
        const fCurr = boundedF(x)
        if (fCurr < fBest) {
            fBest = fCurr
            xBest = [...x]
        }
        const xNew = x.map((_, i) => bounds[i][0] + Math.random() * (bounds[i][1] - bounds[i][0]))
        if (boundedF(xNew) < fCurr || Math.exp((fCurr - boundedF(xNew)) / t) > Math.random()) {
            x = xNew
        }
        t *= alpha
    }
